...
def test_runtime_health_success_fixture_normalizes_omitted_policy_caps() -> None:
    payload = json.loads((FIXTURES / "runtime_health_success.json").read_text())
    parsed = HealthResponse.from_dict(payload)
    expected = dict(payload)
    health = dict(expected["health"])
    device_policy = dict(health["device_policy"])
    device_policy["alert_sms_monthly_cap"] = None
    device_policy["alert_whatsapp_monthly_cap"] = None
    health["device_policy"] = device_policy
